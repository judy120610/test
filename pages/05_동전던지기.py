import streamlit as st
import streamlit.components.v1 as components

elif menu == "🪙 동전 던지기":
    st.title("🪙 운명의 동전 던지기")
    html_code = """
    <style>
        .scene { width: 150px; height: 150px; perspective: 1000px; margin: 50px auto; }
        .coin { width: 100%; height: 100%; position: relative; transform-style: preserve-3d; transition: transform 2s cubic-bezier(0.2, 0.8, 0.3, 1.1); }
        .side { position: absolute; width: 100%; height: 100%; border-radius: 50%; backface-visibility: hidden; 
                display: flex; flex-direction: column; align-items: center; justify-content: center; 
                border: 4px solid #bdc3c7; box-shadow: inset 0 0 20px rgba(0,0,0,0.1); }
        .heads { background: linear-gradient(145deg, #e6e6e6, #cfcfcf); color: #2c3e50; transform: translateZ(5px); }
        .tails { background: linear-gradient(145deg, #d4d4d4, #b0b0b0); transform: rotateY(180deg) translateZ(5px); color: #2c3e50; }
        /* 동전 두께 표현을 위한 중간층 */
        .edge { position: absolute; width: 100%; height: 100%; background: #bdc3c7; border-radius: 50%; transform: translateZ(0); }
        .coin-val { font-size: 40px; font-weight: bold; }
    </style>
    <div class="scene">
        <div id="coin" class="coin">
            <div class="edge"></div>
            <div class="side heads"><div class="coin-val">100</div><div>한국은행</div></div>
            <div class="side tails"><div class="coin-val">👤</div><div>이순신</div></div>
        </div>
    </div>
    <div style="text-align:center;">
        <button onclick="flip()" style="padding:15px 40px; background:#FFDAC1; border:none; border-radius:15px; cursor:pointer; font-size:20px; font-weight:bold;">동전 튕기기!</button>
        <h2 id="res" style="color:#555; margin-top:30px;">준비...</h2>
    </div>
    <script>
        let currentRotation = 0;
        function flip() {
            const coin = document.getElementById('coin');
            const isHeads = Math.random() < 0.5;
            const extraRotation = isHeads ? 1800 : 1980; // 많은 회전수 부여
            currentRotation += extraRotation;
            
            coin.style.transform = `rotateY(${currentRotation}deg)`;
            document.getElementById('res').innerText = "결과는...?";
            
            setTimeout(() => {
                const finalStatus = (currentRotation / 180) % 2 === 0;
                document.getElementById('res').innerText = isHeads ? "결과: 앞면 (100)" : "결과: 뒷면 (이순신)";
            }, 2000);
        }
    </script>
    """
    components.html(html_code, height=500)
